...
export const playerData = pgTable("player_data", {
  ...
  // You can use { mode: "bigint" } if numbers are exceeding js number limitations
  selfMitigatedDamage: bigint("self_mitigated_damage", { mode: "number" })
    .default(0)
    .notNull(),
  // You can use { mode: "bigint" } if numbers are exceeding js number limitations
  damageToTurrets: bigint("damage_to_turrets", { mode: "number" })
    .default(0)
    .notNull(),
  // You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
export const performances = pgTable(
  ...
  (table) => {
    return {
      performancesPlayerIdGameIdKey: unique(
        "performances_player_id_game_id_key",
      ).on(table.playerId, table.gameId),
    };
  },
);
...
export const series = pgTable(
  ...
  (table) => {
    return {
      seriesTeam1IdTeam2IdPlayoffsKey: unique(
        "series_team1_id_team2_id_playoffs_key",
      ).on(table.team1Id, table.team2Id, table.playoffs),
    };
  },
);
...
export const games = pgTable("games", {
  ...
  seriesId: integer("series_id")
    .notNull()
    .references(() => series.id, { onDelete: "set null", onUpdate: "cascade" }),
...
export const teams = pgTable(
  ...
  (table) => {
    return {
      lowerIdx: index("teams_lower_idx").using(
        "btree",
        sql`lower((name)::text)`,
      ),
    };
...
export const standings = pgTable("standings", {
  ...
  teamId: integer("team_id")
    .notNull()
    .references(() => teams.id, { onDelete: "set null", onUpdate: "cascade" }),
});
...
export const divisions = pgTable(
  ...
  (table) => {
    return {
      lowerIdx: index("divisions_lower_idx").using(
        "btree",
        sql`lower((name)::text)`,
      ),
      divisionsTournamentIdKey: unique("divisions_tournament_id_key").on(
        table.tournamentId,
      ),
    };
  },
...
export const players = pgTable(
  ...
  (table) => {
    return {
      primaryRiotPuuidIdx: uniqueIndex("players_primary_riot_puuid_idx").using(
        "btree",
        table.primaryRiotPuuid,
      ),
      playersPrimaryRiotPuuidKey: unique("players_primary_riot_puuid_key").on(
        table.primaryRiotPuuid,
      ),
      playersSummonerNameKey: unique("players_summoner_name_key").on(
        table.summonerName,
      ),
    };
  },
